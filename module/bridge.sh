...
#   [0]=0x03  [1]=使能 0x0F  [2][3]=0  [4]=左马达  [5]=右马达
#   [6]=持续时间(×10ms)  [7]=循环次数  [8]=起始延迟(×10ms)
#
# ⚠️ 左右幅值在 **[4]/[5]**，不是 [2]/[3] —— 这里原来那条注释写错了（写成 [2..5] 是
#    四个幅值、[2]/[3] 是左右），和代码差了两格。描述符里 [2..5] 确实是四个幅值字段
#    （Xbox 系列有主马达 + 扳机马达共四个），但主马达是第 3/4 个字段，也就是字节
#    [4]/[5]；[2]/[3]（扳机马达）本模块不动，留 0。
#    左右不等时就是 [4]≠[5]，例如 左=255 右=64 → `03 0f 00 00 ff 40 <dur> <loop> 00`。
#
    ...
    while :; do
        ...
        case "$line" in
            *"nextStep: sending vibrate deviceId="*)
                ...
                # channels=[...] 的三种形态都要认（框架对输入设备一般给两通道，
                # 但不能假定 —— 单通道时把右声道判成 0 就等于静音右马达）：
                #   两通道  "channels=[0 : 255, 1 : 64]"  → 左=255 右=64
                #   单通道  "channels=[0 : 128]"          → 左=右=128（镜像）
                #   三通道+ "channels=[0 : 1, 1 : 2, 2 : 3]" → 取前两个（0/1 是主马达）
                _ch="${line##*channels=[}"
                _ch="${_ch%%]*}"                 # "0 : 255, 1 : 64"
                _ch="${_ch#*: }"                 # "255, 1 : 64"
                l="${_ch%%,*}"
